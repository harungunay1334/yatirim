...
const ASSET_CONFIG = {
    ...
    // BIST
    'THYAO.IS': { displayName: 'THYAO',               type: 'stock',     yahooTicker: 'THYAO.IS',  isTRY: true },
    'SISE.IS':  { displayName: 'SISE',                type: 'stock',     yahooTicker: 'SISE.IS',   isTRY: true },
    'GARAN.IS': { displayName: 'GARAN',               type: 'stock',     yahooTicker: 'GARAN.IS',  isTRY: true },
    'AKBNK.IS': { displayName: 'AKBNK',               type: 'stock',     yahooTicker: 'AKBNK.IS',  isTRY: true },
    'EREGL.IS': { displayName: 'EREGL',               type: 'stock',     yahooTicker: 'EREGL.IS',  isTRY: true },
    'KCHOL.IS': { displayName: 'KCHOL',               type: 'stock',     yahooTicker: 'KCHOL.IS',  isTRY: true },
    'BIMAS.IS': { displayName: 'BİM',                 type: 'stock',     yahooTicker: 'BIMAS.IS',  isTRY: true },
    'ASELS.IS': { displayName: 'ASELS',               type: 'stock',     yahooTicker: 'ASELS.IS',  isTRY: true },
...
/** CORS proxy — Yahoo Finance direkt CORS izni vermez */
...
const el = {
    ...
    customTicker:    document.getElementById('customTicker'),
...
function bindEvents() {
    ...
    // Varlık kategorisi değişikliği (Hazır Liste / Manuel Giriş)
    el.assetCategory.addEventListener('change', () => {
        const isCustom = el.assetCategory.value === 'custom';
        el.presetGroup.classList.toggle('hidden', isCustom);
        el.customGroup.classList.toggle('hidden', !isCustom);
        if (isCustom) el.customTicker.focus();
    });
...
function getSelectedTicker() {
    if (el.assetCategory.value === 'custom') {
        const raw = el.customTicker.value.trim().toUpperCase();
        return raw || null;
    }
    return el.assetSelect.value;
}
 ...
 * @param {string} ticker - Varlık kodu (ASSET_CONFIG'den ya da özel)
 */
async function fetchPriceForTicker(ticker) {
    ...
    try {
        const url    = `${PROXY}${encodeURIComponent(`https://query1.finance.yahoo.com/v8/finance/chart/${yahooTick}?interval=1d&range=1d`)}`;
        ...
        if (!meta) throw new Error('Veri bulunamadı');

        // Yahoo Finance, regularMarketPrice'ı verir
        let priceUSD = meta.regularMarketPrice ?? meta.previousClose;

        if (!priceUSD || priceUSD <= 0) throw new Error('Geçersiz fiyat');

        // TL'ye çevirme mantığı
        let priceTRY;

        if (config?.isTRY) {
            // BIST hisseleri zaten TL
            priceTRY = priceUSD;

        } else if (config?.isGoldGram) {
            // Altın: troy ounce → gram (1 troy oz = 31.1035 gram), USD → TRY
            const usdTry = state.usdTry || state.prices['USD-TRY'] || 1;
        ...
        } else if (ticker === 'USD-TRY' || yahooTick === 'USDTRY=X') {
            // USD/TRY doğrudan TL
            priceTRY        = priceUSD;
            state.usdTry    = priceUSD; // globale kaydet

        } else if (meta.currency === 'TRY' || yahooTick.endsWith('-TRY')) {
            // Zaten TL bazında (kripto-TRY, forex-TRY)
            priceTRY = priceUSD;

        } else {
            // USD cinsinden ABD hissesi → TRY'e çevir
            const usdTry = state.usdTry || state.prices['USD-TRY'] || 1;
        ...
        console.warn(`Fiyat çekilemedi [${ticker}]:`, err.message);
        // Önceki fiyatı koru (fallback)
